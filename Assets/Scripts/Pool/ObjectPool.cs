...
//where T : MonoBehaviour  // T 타입은 MonoBehaviour이거나 RecycleObject를 상속받은 클래스만 가능하다.->조건부여
public class ObjectPool<T> : MonoBehaviour where T : RecycleObject
{
    ...
    public void Initialize()
    {
        ...
        else
        {
            ...
            foreach( T obj in pool )        // 특정 컬렉션 안에 있는 모든 요소를 한번씩 처리해야 할 일이 있을 때 사용[in 안에]
            {
    ...
    public T GetObject()
    {
        if( readyQueue.Count > 0 )              // 레디큐에 오브젝트가 남아있는지 확인
        {
            ...
            comp.gameObject.SetActive(true);    // 활겅화 시키고
            return comp;                        // 리턴
        }
        else
        {
    ...
    void GenerateObjects(int start, int end, T[] results)
    {
        for (int i = start; i < end; i++)
        {
            ...
            comp.onDisable += () => readyQueue.Enqueue(comp);               // 재활용 오브젝트가 비활성화 되면 레디큐러 더;덜랴리
            //readyQueue.Enqueue(comp);                                       // 레디큐에 추가하고(위에 델리게이트 등록한 것 때문에 아래에서 비활성화하면 자동으로 처리)
