...
public class Background : MonoBehaviour
{
    ...
    /// 그림 가로 길이
    /// </summary>
    const float BackgroundWidth = 13.6f;        //가로길이
    ...
    protected virtual void Awake()
    {
        ...
        {
            bgSlots[i] = transform.GetChild(i);                 // 배열에 자식을 하니씩 넣기
        }
    ...
    private void Update()
    {
        for(int i = 0; i < bgSlots.Length;i++)                  // 배열 만들고
        {
            bgSlots[i].Translate(Time.deltaTime * scrollingSpeed * -transform.right);   // 이동대상을 계속 왼쪽으로 이동시키기

            if (bgSlots[i].position.x < baseLineX)              // 기준선을 넘었는지 확인
            {
